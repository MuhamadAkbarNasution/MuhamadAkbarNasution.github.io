document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeNavigation = () => {
        ...
        navLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                // Jangan non-aktifkan link logout
    ...
    // Mengatur tampilan form login/signup
    const setupAuthView = (isLogin) => {
        authTitle.textContent = isLogin ? 'Login' : 'Sign Up';
        loginBtn.style.display = isLogin ? 'block' : 'none';
        signupBtn.style.display = isLogin ? 'none' : 'block';
        signupPrompt.style.display = isLogin ? 'block' : 'none';
        loginPrompt.style.display = isLogin ? 'none' : 'block';
    ...
    loginBtn.addEventListener('click', () => {
        ...
        if (!email || !password) {
            authError.textContent = 'Mohon masukkan email dan password.';
            return;
        }
            ...
            .catch((error) => {
                 switch (error.code) {
                    ...
                    case 'auth/weak-password':
                        authError.textContent = 'Password harus terdiri dari minimal 6 karakter.';
                        break;
                    default:
                        authError.textContent = 'Gagal membuat akun. Coba lagi.';
                        break;
                }
            });
    });
    ...
    auth.onAuthStateChanged(user => {
        const existingUserInfo = document.getElementById('user-info-li');
        if(existingUserInfo) existingUserInfo.remove();
        const existingLogout = document.getElementById('logout-li');
        ...
        if (user) {
            ...
            userInfoLi.id = 'user-info-li';
            userInfoLi.innerHTML = `<span id="user-email-display" title="${user.email}">${user.email}</span>`;

            const logoutLi = document.createElement('li');
            logoutLi.id = 'logout-li';
            logoutLi.innerHTML = `<a href="#" id="logout-btn"><i class="fas fa-sign-out-alt"></i> Logout</a>`;
            
            mainNav.appendChild(userInfoLi);
            mainNav.appendChild(logoutLi);
            ...
            if (!body.classList.contains('app-initialized')) {
                initializeApp();
            ...
            setupAuthView(true); // Tampilkan form login default
    ...
    const initializeApp = () => {
        ...
        // Timeout disingkat agar aplikasi lebih cepat muncul setelah login
        setTimeout(() => {
            renderAllProductShowcases();
            renderCart();
            renderFavorites();
            ...
            body.classList.remove('no-scroll');
        }, 500); // Mengurangi delay dari 1200ms ke 500ms
    };
    ...
    const startApp = () => {
        ...
        // Listener `onAuthStateChanged` akan otomatis menjalankan langkah berikutnya
    };
    ...
    window.addEventListener('scroll', () => {
        // Hanya jalankan jika aplikasi utama terlihat
        if (mainAppContent.style.display === 'block') {
    ...
    if (loadingVideo) {
        ...
        loadingVideo.addEventListener('error', startApp); // Fallback jika video gagal
    } else {
        startApp(); // Langsung mulai jika tidak ada video
    }
});