document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        id: {
            ...
            sidebar_shipping_estimator: 'Estimasi Ongkir',
        shipping_destination_label: 'Kecamatan, Kota',
        shipping_calculate_btn: 'Cek Estimasi',
        shipping_result_jabodetabek: 'Estimasi ongkir ke <strong>{destination}</strong> adalah <strong>Rp5.000 - Rp15.000</strong>. Detail akan dikonfirmasi Admin.',
        shipping_result_java: 'Estimasi ongkir ke <strong>{destination}</strong> adalah <strong>Rp10.000 - Rp25.000</strong>. Detail akan dikonfirmasi Admin.',
        shipping_result_outside: 'Estimasi ongkir ke <strong>{destination}</strong> adalah <strong>Rp20.000 - Rp95.000</strong>. Detail akan dikonfirmasi Admin.',
        shipping_error_input: 'Mohon masukkan tujuan pengiriman (Kecamatan, Kota).',
        sidebar_size_calculator: 'Kalkulator Ukuran Ideal',
        height_label: 'Tinggi Badan (cm)',
        weight_label: 'Berat Badan (kg)',
        calculate_size_btn: 'Cek Ukuran',
        size_error_input: 'Mohon masukkan tinggi dan berat badan.',
        size_recommendation: (size) => `Berdasarkan data Anda, kami merekomendasikan ukuran <strong>${size}</strong>.`,
        sidebar_fit_ship: 'Fit & Ship',
        shipping_estimator_title: 'Estimasi Ongkir',
        size_calculator_title: 'Kalkulator Ukuran Ideal',
        ...
        en: {
            ...
            follow_instagram: 'Instagram',
            follow_tiktok: 'TikTok',
            sidebar_shipping_estimator: 'Shipping Cost Estimation',
        shipping_destination_label: 'District, City',
        shipping_calculate_btn: 'Check Estimation',
        shipping_result_jabodetabek: 'Shipping estimate to <strong>{destination}</strong> is <strong>IDR 5,000 - IDR 15,000</strong>. Details will be confirmed by Admin.',
        shipping_result_java: 'Shipping estimate to <strong>{destination}</strong> is <strong>IDR 10,000 - IDR 25,000</strong>. Details will be confirmed by Admin.',
        shipping_result_outside: 'Shipping estimate to <strong>{destination}</strong> is <strong>IDR 20,000 - IDR 95,000</strong>. Details will be confirmed by Admin.',
        shipping_error_input: 'Please enter a shipping destination (District, City).',
        sidebar_size_calculator: 'Ideal Size Calculator',
        height_label: 'Height (cm)',
        weight_label: 'Weight (kg)',
        calculate_size_btn: 'Check Size',
        size_error_input: 'Please enter both height and weight.',
        size_recommendation: (size) => `Based on your data, we recommend size <strong>${size}</strong>.`,
        sidebar_fit_ship: 'Fit & Ship',
        shipping_estimator_title: 'Shipping Estimator',
        size_calculator_title: 'Ideal Size Calculator',
    ...
    const shippingDestinationInput = document.getElementById('shipping-destination-input');
    const calculateSizeBtn = document.getElementById('calculate-size-btn');
    const heightInput = document.getElementById('height-input');
    const weightInput = document.getElementById('weight-input');
    const calculatorResultModal = document.getElementById('calculator-result-modal');
    const calculatorResultMessage = document.getElementById('calculator-result-message');
    const closeResultModalBtn = document.getElementById('close-result-modal-btn');
    const fitShipContainer = document.querySelector('.fit-ship-container');
    if (fitShipContainer) {
    fitShipContainer.addEventListener('click', (e) => {
   ...
   const updateShippingEstimates = () => {
    ...
    const showCalculatorResult = (message) => {
    if (calculatorResultModal && calculatorResultMessage) {
        calculatorResultMessage.innerHTML = message;
        openModal(calculatorResultModal);
    }
};
...
const updateFilterUI = () => {
    ...
    const setLanguage = (lang) => {
        ...
        document.querySelectorAll('[data-lang-key="sidebar_fit_ship"]').forEach(el => el.innerHTML = `${translations[lang].sidebar_fit_ship} <i class="fas fa-chevron-right"></i>`);
        document.querySelectorAll('[data-lang-key="shipping_estimator_title"]').forEach(el => el.textContent = translations[lang].shipping_estimator_title);
        document.querySelectorAll('[data-lang-key="size_calculator_title"]').forEach(el => el.textContent = translations[lang].size_calculator_title);
    ...
    if (calculateShippingBtn) {
        calculateShippingBtn.addEventListener('click', () => {
            const destination = shippingDestinationInput.value.trim().toLowerCase();
            
            if (!destination) {
    showCalculatorResult(translations[currentLanguage].shipping_error_input);
    return;
}
            ...
            const pulauJawa = [
                ...
                'jawa barat', 'bandung', 'cirebon', 'sukabumi', 'tasikmalaya',
                'jawa tengah', 'semarang', 'surakarta', 'solo', 'magelang', 'pekalongan', 'tegal',
                'yogyakarta', 'yogya', 'sleman', 'bantul',
                'jawa timur', 'surabaya', 'malang', 'sidoarjo', 'kediri', 'jember'
            ];

            const isJabodetabek = jabodetabek.some(city => destination.includes(city));
            const isPulauJawa = pulauJawa.some(city => destination.includes(city));

            let resultKey;
            if (isJabodetabek) {
                resultKey = 'shipping_result_jabodetabek';
            } else if (isPulauJawa) {
                resultKey = 'shipping_result_java';
            } else {
                resultKey = 'shipping_result_outside';
            }
            
            let resultText = translations[currentLanguage][resultKey].replace('{destination}', shippingDestinationInput.value.trim());

            shippingResultEl.innerHTML = resultText;
            shippingResultEl.classList.add('show');
        });
    }
    ...
    if (calculateSizeBtn) {
        calculateSizeBtn.addEventListener('click', () => {
            ...
            if (!height || !weight) {
    showCalculatorResult(translations[currentLanguage].size_error_input);
    return;
}
            ...
            if (height < 160 && weight < 50) {
                ...
                recommendedSize = 'S';
            } else if ((height >= 170 && height <= 175) && (weight >= 60 && weight <= 65)) {
                recommendedSize = 'L';
            } else if (height > 175 && weight > 65) {
                recommendedSize = 'XL';
            } else if ((height >= 160 && height <= 170) && (weight > 55 && weight <= 60)) {
            ...
            sizeResultEl.innerHTML = translations[currentLanguage].size_recommendation(recommendedSize);
            sizeResultEl.classList.add('show');
        });
    }
...
if (paymentMethodContainer) {
    ...
    closeResultModalBtn.addEventListener('click', () => {
        closeModal(calculatorResultModal);
    });
}
